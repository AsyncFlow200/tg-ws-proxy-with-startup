...
# ------------------- ПОСТ-СБОРКА (автозагрузка) -------------------
# Этот код выполняется ПОСЛЕ создания .exe файла
exe_path = os.path.abspath("dist/TgWsProxy.exe")
startup_folder = os.path.expandvars(r"%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup")

if os.path.exists(exe_path):
    dest_path = os.path.join(startup_folder, "TgWsProxy.exe")
    try:
        shutil.copy2(exe_path, dest_path)
        print(f"[+] Скопировано в автозагрузку: {dest_path}")
    except Exception as e:
        print(f"[-] Ошибка копирования: {e}")
else:
    print("[-] Файл dist/TgWsProxy.exe не найден, автозагрузка не добавлена")